<script>

</script>

<footer class="text-white">
    <div class="max-w-6xl mx-auto px-4 py-8 md:py-12">
      <!-- Three Columns Section -->
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">

        <div class=text-center>
          <h2 class="font-bold text-xl mb-4">Navigation</h2>
          <ul>
            <li><a href="/" class="hover:text-blue-300 text-xl">Hjem</a></li>
            <li><a href="/priser" class="hover:text-blue-300 text-xl">Priser</a></li>
            <li><a href="/kontakt" class="hover:text-blue-300 text-xl">Kontakt</a></li>
            <li><a href="/om-os" class="hover:text-blue-300 text-xl">Om os</a></li>
          </ul>
        </div>


        <div class="text-center">
          <h2 class="font-bold text-xl mb-4">Information</h2>
          <p class="text-lg"><span class="fas fa-map-marker-alt"></span> Køge, 4600 Danmark</p>
          <p class="text-lg"><span class="fas fa-envelope"></span> bilperfektionisten@gmail.com</p>
          <p class="text-lg"><span class="fas fa-phone"></span> 22 94 25 54</p>
        </div>
  
 
        <div class="text-center">
          <h2 class="font-bold text-xl mb-4">Telefontider</h2>
          <p class="text-lg"><span class="fas fa-clock mr-2"></span>Mandag til fredag: 8:00-16:00</p>
          <p class="text-lg">&ensp; &emsp; &nbsp; &nbsp; Lørdag og søndag: 10:00-16:00</p>
        </div>
      </div>
  

      <div class="mt-8 pt-8 md:pt-12 border-t border-gray-700">
        <p class="text-center text-gray-400">&copy; 2024 Bilperfektionisten. Alle rettigheder forbeholdes.</p>
       <!--
        <br>
        <p class="text-center text-sm text-gray-400">Website af magnus.danielsen@hotmail.com</p>
        -->
      </div>
    </div>
</footer>

<style>
footer {
  background-color: #35475D;
}
</style>
  